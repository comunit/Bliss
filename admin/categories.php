<?php
  ...
  if(isset($_GET['hide']) && !empty($_GET['hide'])) {
    ...
    $hide = sanitize($hide_id);
    $hide_result = $db->query("UPDATE categories SET hide = 1 WHERE id = '{$hide_id}'");
    $hide_category = mysqli_fetch_assoc($hide_result);
...
<div class="row">
  ...
  <!-- Form -->
  <div class="col-md-6">
    <form class="form" action="categories<?php echo ((isset($_GET['edit']))?'?edit='.$edit_id : ''); ?>" method="post">
  ...
  <div class="col-md-6">
    <table class="table table-bordered table-condensed">
      ...
      <tbody>
        <?php $result = $db->query("SELECT * FROM categories WHERE parent = 0 AND hide = 0"); ?>
        <?php while($parent = mysqli_fetch_assoc($result)) : ?>
        <?php
          $parent_id = (int)$parent['id'];
          $cresult = $db->query("SELECT * FROM categories WHERE parent = '{$parent_id}' AND hide = 0");
        ?>
        <tr class="bg-primary">
          ...
          <td>
            ...
            <a class="btn btn-xs btn-default" href="categories.php?hide=<?php echo $parent['id']; ?>"><span class="glyphicon glyphicon-minus"></span></a>
          </td>
        </tr>
          <?php while($child = mysqli_fetch_assoc($cresult)) : ?>
            <tr class="bg-info">
              ...
              <td>
                ...
                <a class="btn btn-xs btn-default" href="categories.php?hide=<?php echo $child['id']; ?>"><span class="glyphicon glyphicon-minus"></span></a>
              </td>
            </tr>
          <?php endwhile; ?>
        <?php endwhile; ?>
